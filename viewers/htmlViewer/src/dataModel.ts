...
export type MappingKeyValuePair = {
    key: number;
    value: string;
}
...
export const MIDI_RPN_SOURCE_TYPE_KEY = 7;
...
function genMidiDeviceChannelSourceFunctions() {
    ...
    for (let key = 0; key <= 15; key++) {
        sourceFunctions.push({ key: key, value: `TRS Midi Channel ${key + 1}` });
    }
    for (let key = 16; key <= 31; key++) {
        sourceFunctions.push({ key: key, value: `USB Host Midi Channel ${key - 15}` });
    }
    for (let key = 32; key <= 47; key++) {
        sourceFunctions.push({ key: key, value: `USB Device Midi Channel ${key - 31}` });
    }
...
function genMidiNoteExtras() {
    ...
    for (let key = 5; key <= 124; key++) {
        extras.push({ key: key, value: `Selected Midi Note ${key - 5} On/Off` });
    }
    for (let key = 125; key <= 245; key++) {
        extras.push({ key: key, value: `Selected Midi Note ${key - 125} Velocity` });
    }
    for (let key = 246; key <= 364; key++) {
        extras.push({ key: key, value: `Selected Midi Note ${key - 246} Aftertouch` });
    }
    return extras;
}
...
function genVariableSourceFunctions() {
    ...
    sourceFunctions.push({ key: 65535, value: "----" });
    for (let key = 0; key <= 15; key++) {
        sourceFunctions.push({ key: key, value: `Variable ${String.fromCharCode(65 + key)}` });
    }
    for (let key = 16; key <= 86; key++) {
        sourceFunctions.push({ key: key, value: `Mapping Row ${(key - 16).toString(16)} (${key - 16})` });
    }
    return sourceFunctions;
}
...
function genSkipSourceFunctions() {
    ...
    sourceFunctions.push({ key: 65535, value: "----" });
    let key = 0;
    for (let rowSkipCount = 1; rowSkipCount <= 16; rowSkipCount++) {
        sourceFunctions.push({ key: key++, value: `Skip ${rowSkipCount} Rows If Param1 < Param2` });
        sourceFunctions.push({ key: key++, value: `Skip ${rowSkipCount} Rows If Param1 <= Param2` });
        sourceFunctions.push({ key: key++, value: `Skip ${rowSkipCount} Rows If Param1 > Param2` });
        sourceFunctions.push({ key: key++, value: `Skip ${rowSkipCount} Rows If Param1 >= Param2` });
        sourceFunctions.push({ key: key++, value: `Skip ${rowSkipCount} Rows If Param1 = Param2` });
...
function genControlSourceExtras() {
    ...
    for (let key = 0; key <= 31; key++) {
        extras.push({ key: key, value: `Execute ${key + 1} Mapping Rows` });
    }
    return extras;
}
...
function genVariableDestinationFunctions() {
    ...
    for (let key = 0; key <= 15; key++) {
        destinationFunctions.push({ key: key, value: `Variable ${String.fromCharCode(65 + key)}` });
    }
    for (let key = 16; key <= 85; key++) {
        destinationFunctions.push({ key: key, value: `Mapping Row ${(key - 16).toString(16)}h (${key - 16})` });
    }
...
function genMidiDeviceChannelDestinationFunctions(addLearn = true) {
    ...
    destinationFunctions.push({ key: 65535, value: "----" });
    for (let key = 0; key <= 15; key++) {
        destinationFunctions.push({ key: key, value: `TRS Midi Channel ${key + 1}` });
    }
    for (let key = 16; key <= 31; key++) {
        destinationFunctions.push({ key: key, value: `USB Host Midi Channel ${key - 15}` });
    }
    for (let key = 32; key <= 47; key++) {
        destinationFunctions.push({ key: key, value: `USB Device Midi Channel ${key - 31}` });
    }
    if (addLearn) { destinationFunctions.push({ key: 48, value: "MIDI Learn" }); }
    return destinationFunctions;
}
...
function genCVDestinationExtras() {
    ...
    for (let key = 0; key <= 5; key++) {
        extras.push({ key: key, value: `Local CV ${key}` });
    }
    for (let key = 6; key <= 11; key++) {
        extras.push({ key: key, value: `Local MOD ${key - 6}` });
    }
    for (let key = 12; key <= 27; key++) {
        extras.push({ key: key, value: `CV16 NSA1 ${key - 12}` });
    }
    for (let key = 28; key <= 43; key++) {
        extras.push({ key: key, value: `CV16 NSA2 ${key - 28}` });
    }
    for (let key = 44; key <= 59; key++) {
        extras.push({ key: key, value: `CV16 NSA3 ${key - 44}` });
    }
    for (let key = 60; key <= 75; key++) {
        extras.push({ key: key, value: `CV16 NSA4 ${key - 60}` });
    }
    return extras;
}
...
function genTriggerDestinationExtras() {
    ...
    extras.push({ key: 65535, value: "----" });
    for (let key = 0; key <= 5; key++) {
        extras.push({ key: key, value: `Local Trigger ${key}` });
    }
    for (let key = 6; key <= 21; key++) {
        extras.push({ key: key, value: `Trigger16 NSA1 ${key - 6}` });
    }
    for (let key = 22; key <= 37; key++) {
        extras.push({ key: key, value: `Trigger16 NSA2 ${key - 22}` });
    }
    for (let key = 38; key <= 53; key++) {
        extras.push({ key: key, value: `Trigger16 NSA3 ${key - 38}` });
    }
    for (let key = 54; key <= 69; key++) {
        extras.push({ key: key, value: `Trigger16 NSA4 ${key - 54}` });
    }
    return extras;
}
...
function genTrackDestinationFunctions() {
    ...
    for (let key = 38; key <= 53; key++) {
        destinationFunctions.push({ key: key, value: `Drum Matrix Euclidean ${key - 38}` });
    }
    ...
    for (let key = 56; key <= 119; key++) {
        destinationFunctions.push({ key: key, value: `Note at Pattern Step ${key - 56}` });
    }
    for (let key = 120; key <= 183; key++) {
        destinationFunctions.push({ key: key, value: `Value at Cursor Column Step ${key - 120}` });
    }
    return destinationFunctions;
}
...
function genTableDestinationFunctions() {
    ...
    destinationFunctions.push({ key: 65535, value: "----" });
    for (let key = 0; key <= 31; key++) {
        destinationFunctions.push({ key: key, value: `Selected Table ${key + 1}` });
    }
    for (let key = 32; key <= 40; key++) {
        destinationFunctions.push({ key: key, value: `Running Table on Track ${key - 31}` });
    }
    return destinationFunctions;
}
...
function genRandomRangeDestinationFunctions() {
    ...
    destinationFunctions.push({ key: 65535, value: "----" });
    for (let key = 0; key <= 15; key++) {
        destinationFunctions.push({ key: key, value: `Range ${key}` });
    }
    return destinationFunctions;
}
...
function genCV16DestinationFunctions() {
    ...
    for (let envCount = 0; envCount <= 7; envCount++) {
        destinationFunctions.push({ key: key++, value: `Envelope ${envCount + 1} Attack` });
        destinationFunctions.push({ key: key++, value: `Envelope ${envCount + 1} Amplitude` });
        destinationFunctions.push({ key: key++, value: `Envelope ${envCount + 1} Release` });
        destinationFunctions.push({ key: key++, value: `Envelope ${envCount + 1} Fire Oneshot` });
        destinationFunctions.push({ key: key++, value: `Envelope ${envCount + 1} Fire A/R` });
    }
    for (let lfoCount = 0; lfoCount <= 7; lfoCount++) {
        destinationFunctions.push({ key: key++, value: `LFO ${lfoCount + 1} Type` });
        destinationFunctions.push({ key: key++, value: `LFO ${lfoCount + 1} Speed` });
        destinationFunctions.push({ key: key++, value: `LFO ${lfoCount + 1} Amplitude` });
        destinationFunctions.push({ key: key++, value: `LFO ${lfoCount + 1} Start` });
        destinationFunctions.push({ key: key++, value: `LFO ${lfoCount + 1} Reset` });
    }
...
function genCV16DestinationExtras() {
    ...
    extras.push({ key: 65535, value: "----" });
    for (let key = 0; key <= 15; key++) {
        extras.push({ key: key, value: `NSA1 ${key + 1}` });
    }
    for (let key = 16; key <= 31; key++) {
        extras.push({ key: key, value: `NSA2 ${key - 15}` });
    }
    for (let key = 32; key <= 47; key++) {
        extras.push({ key: key, value: `NSA3 ${key - 31}` });
    }
    for (let key = 48; key <= 63; key++) {
        extras.push({ key: key, value: `NSA4 ${key - 47}` });
    }
    return extras;
}
...
function genMidiControllerExtras() {
    ...
    for (let key = 0; key <= 127; key++) {
        extras.push({ key: key, value: `MIDI Controller #${key}` });
    }
    return extras;
}
...
function genI2cPortExtras() {
    ...
    extras.push({ key: 65535, value: "----" });
    for (let key = 0; key <= 127; key++) {
        extras.push({ key: key, value: `Port Number ${key}` });
    }
    return extras;
}
